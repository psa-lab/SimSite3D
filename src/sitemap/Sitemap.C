...
Sitemap::Sitemap(const GenPointsParameters& args)
{
  ...
  find_binding_site_hbonds(args.hbond_method, 
                           args.install_dir + "/SimSite3DSoftParams", args.waters,
...
//  delete A_site_atoms;
  ...
  out << "<!--\n"
      ...
      << "# 8) Hbond point number (for the given atom) - SimSite3D internal "
      << "use\n"
      << "# 9) Point position  (SimSite3D internal use)\n"
      << "# 10) Carbon neighbor atom number  (SimSite3D internal use)\n"
      << "-->\n";
...
Sitemap::build_points_header(const GenPointsParameters& args)
{
  ...
  header << REMARK_10 << "Sitemap generated by SimSite3D\n"
